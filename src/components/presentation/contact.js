...
function Contact(props) {
   ...
   const onchange=(event)=>{
    ...
    const onSubmit= async()=>{
        ...
        if(keys.length==0) props.setContact(contact);
        else props.updateContact(contact);
    ...
    return (
          <div className="container med contact" style={{marginBottom:"5rem"}}>
            <div className="section funnel-section">
                ...
                <div className="preview-card">
                    <ResumePreview educationSection={props.education} contactSection={contact} skinCd={props?.document?.skinCd}></ResumePreview>
                </div>
            </div>
        </div>
    );
